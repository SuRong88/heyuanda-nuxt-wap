    ...
    <div class="product-page">
        ...
        <no-ssr>
            <swiper class="product-swiper" :options="swiperOption" ref="productSwiper">
                <!-- 正常 -->
                <swiper-slide v-for="(item, index) in list[currentIndex]" class="product-swiper-item product-swiper-item01" :class="{ active: item.show }">
        ...
        return {
            ...
            swiperOption: {
                ...
                on: {
                    init: function() {
                        setTimeout(() => {
                            that.list[that.currentIndex][initialIndex].show = true;
                        }, 1000);
                    },
                    // 只有0时候不触发slideChangeTransitionEnd 
                    slideChangeTransitionEnd: function() {
                        if(this.activeIndex == initialIndex){
                            setTimeout(()=>{
                                that.list[that.currentIndex][this.activeIndex].show = true;
                            },1000)
                        } else {
                            that.list[that.currentIndex][this.activeIndex].show = true;
                        }
