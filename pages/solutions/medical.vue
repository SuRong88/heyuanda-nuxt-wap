    ...
    <main>
      ...
      <div class="content">
        <section class="sec01">
          ...
          <p class="con-txt">
            基于和元达大数据分析平台，通过建立信息化、标准化、智能化的决策支持应用系统，方便医疗管理机构及医疗服务机构进行卫生管理和决策指导。
          </p>
    ...
    data() {
      return {
        ...
        list: [{
            ...
            tit: '基础设施层',
            desc: '基础设施层由专为大数据定制的服务器及网络构成的集群组成，是构建大数据应用的硬件平台。数据源所涉及的对象包括医疗机构、社康、门诊，以及区公卫机构，医疗机构的HIS、LIS、PACS、CIS；社康HIS、CIS、CHSS以及公卫疾控系统、传染病上传系统等数据构成总体架构最基本数据源，数据源通过医疗机构、社康、公卫等的前置机进行采集，再通过前置机内置路由网关传输到数据平台，数据源层完成了各个分散系统的数据采集及传输。'
          },
          {
          ...
          },
          {
            ...
            tit: '数据处理层',
            desc: '数据处理层对数据进行处理，其处理内容包含：数据清洗、数据校验、数据转换。处理对象包含标准数据及非标准数据，处理的数据结构包括非结构化数据、半结构化数据及结构化数据（如非结构化数据的简单分析--过滤和统计、基于计算模型进行结构化数据和非结构化数据的统一关联和统计处理、复杂的分析和挖掘以及复杂问题的近似模糊求解等操作）。被处理的数据最终转化成符合医疗卫生信息化标准的卫生综合数据，标准数经过ETL后存入医疗中心数据仓库，为大数据应用做数据准备。'
          },
          {
            path: require("@/assets/images/rest/medical/img04.png"),
            tit: '数据应用层',
            desc: '数据应用层为大数据应用平台提供类型丰富的访问接口，包括Search API、Pig、Hive QL以及专为用户设计的SJDBC（类JDBC）和UDF（用户自定义函数）接口，方便用户的使用。'
          }
        ]
      };
    }
